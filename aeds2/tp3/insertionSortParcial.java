...
public class insertionSort{
	...
	public static void generateLog(int comparacoes, int movimentacoes, long tempoNano){
	    try{
	        FileWriter fw = new FileWriter("903104_insercao.txt");
	...
	public void insertionSort(ColecaoRestaurantes colecao){
		Restaurante []restaurantes = colecao.getRestaurantes();
		for(int i = 1; i < colecao.getTamanho(); i++){
			...
			for(j = i-1; j >= 0; j--){
				comparacoes++;
				if((tmp.getCidade().compareTo(restaurantes[j].getCidade())) >= 0){
					break;	
				}
	...
	public static void main(String[] args){
		...
		is.insertionSort(filtrados);
		long fim = System.nanoTime();
		long tempo = fim-inicio;

		generateLog(is.comparacoes, is.movimentacoes, tempo);
