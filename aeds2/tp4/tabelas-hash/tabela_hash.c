...
typedef struct Hash {
    Restaurante *table[TAM_TABELA + TAM_RESERVA];
    int reserva;
} Hash;

void init_hash(Hash *h) {
    h->reserva = 0;
    for (int i = 0; i < TAM_TABELA + TAM_RESERVA; i++) {
        h->table[i] = NULL;
    }
}
...
void inserir_hash(Hash *h, Restaurante *r) {
    ...
    if (h->table[pos] == NULL) {
        ...
        if (h->reserva < TAM_RESERVA) {
            h->table[TAM_TABELA + h->reserva] = r;
            h->reserva++;
        } else {
            printf("%s\n", r->nome);
        }
...
int pesquisar_hash(Hash *h, char *nome) {
    int pos = hash(nome);
    if (h->table[pos] != NULL && strcmp(h->table[pos]->nome, nome) == 0) {
        return pos;
    }
    for (int i = 0; i < h->reserva; i++) {
        if (strcmp(h->table[TAM_TABELA + i]->nome, nome) == 0) {
            return TAM_TABELA + i;
        }
    }
    return -1;
...
int main() {
    ...
    while (fgets(entrada, sizeof(entrada), stdin) != NULL) {
        ...
        if (length_str(entrada) > 0) {
            int pos = pesquisar_hash(&h, entrada);
            if (pos != -1) {
                char buffer[1500];
                formatar_restaurante(h.table[pos], buffer);
