...
void quick_sort_rec(Colecao_Restaurantes * colecao, int esq, int dir){
	int i = esq, j = dir;
	int pivo = (esq+dir)/2;
	double pivo_av = colecao->restaurantes[pivo]->avaliacao;
	char *pivo_nome = colecao->restaurantes[pivo]->nome;
	while(i<=j){
		while(colecao->restaurantes[i]->avaliacao < pivo_av || (colecao->restaurantes[i]->avaliacao == pivo_av && strcmp(colecao->restaurantes[i]->nome, pivo_nome) < 0)){
			i++;
			comparacoes++;
		}
                while(colecao->restaurantes[j]->avaliacao > pivo_av || (colecao->restaurantes[j]->avaliacao == pivo_av && strcmp(colecao->restaurantes[j]->nome, pivo_nome) > 0)){
                        j--;
			comparacoes++;
                }
		...
		if(i <= j){
			...
			colecao->restaurantes[i] = colecao->restaurantes[j];
			colecao->restaurantes[j] = tmp;
			i++;
			j--;
			movimentacoes+=3;
		}
	...
	if(esq < j){
		quick_sort_rec(colecao, esq, j);
	}
	
	if(i < dir){
		quick_sort_rec(colecao, i, dir);
	}
}

void quick_sort(Colecao_Restaurantes *colecao){
	if(colecao->tamanho > 0){
		quick_sort_rec(colecao, 0, colecao->tamanho-1);
	}
}
...
void generate_log(double tempo){
	FILE *log = fopen("903104_quicksort.txt", "w");
