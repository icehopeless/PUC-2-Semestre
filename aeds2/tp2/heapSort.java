...
public class heapSort{
	...
	public static void generateLog(int comparacoes, int movimentacoes, long tempoNano){
	    try{
	        FileWriter fw = new FileWriter("903104_heapsort.txt");
	        double tempoSegundos = tempoNano / 1_000_000_000.0;
		
	        fw.write("903104\t" + comparacoes + "\t" + movimentacoes + "\t" + tempoSegundos);
	        fw.close();
	...
	private int comparar(Restaurante r1, Restaurante r2) {
		comparacoes++;
	...
	public void sort(ColecaoRestaurantes c){
		int n = c.getTamanho();
		Restaurante []restaurantes = c.getRestaurantes();
		...
		//heapify - 
		for(int j = n-1; j > 0; j--){
			Restaurante tmp = restaurantes[0];
			restaurantes[0] = restaurantes[j];
			restaurantes[j] = tmp;
			movimentacoes+=3;
	...
	//monta o heap recursivamente, trocando filho maior que raiz
	private void heapSort(ColecaoRestaurantes c, int n, int i){
		int raiz = i;
		int esq = 2*i+1;
		int dir = 2*i+2;
		Restaurante []restaurantes = c.getRestaurantes();
		...
		if(esq < n && comparar(restaurantes[esq], restaurantes[raiz]) > 0){
			raiz = esq;
		}
                if(dir < n && comparar(restaurantes[dir], restaurantes[raiz]) > 0){
                        raiz = dir;
                }
		
		//se a raiz mudou (houve troca)
		if(raiz != i){
			Restaurante tmp = restaurantes[i];
			restaurantes[i] = restaurantes[raiz];
			restaurantes[raiz] = tmp;
			movimentacoes+=3;
			heapSort(c, n, raiz);
	...
	public static void main(String[] args){
		...
		ColecaoRestaurantes filtrados = new ColecaoRestaurantes();
		...
		while(scan.hasNextInt()){
			...
			filtrados.add(colecao.getRestaurantes()[n-1]);	
		}
		...
		generateLog(hs.comparacoes, hs.movimentacoes, tempo);


   		Restaurante[] restaurantes = filtrados.getRestaurantes();
    		for(int i = 0; i < filtrados.getTamanho(); i++){
