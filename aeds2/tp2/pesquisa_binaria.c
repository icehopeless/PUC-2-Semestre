...
void select_sort(Colecao_Restaurantes * colecao){
	for(int i = 0; i < colecao->tamanho; i++){
		int menor = i;
		for(int j = i; j < colecao->tamanho; j++){
			if(strcmp(colecao->restaurantes[menor]->nome, colecao->restaurantes[j]->nome) > 0){
				menor = j;
			}
		}
	Restaurante *tmp = colecao->restaurantes[i];
        colecao->restaurantes[i] = colecao->restaurantes[menor];
        colecao->restaurantes[menor] = tmp;

	}
}

int pesquisa_binaria(Colecao_Restaurantes * colecao, char * chave){
	int meio;
	int esq = 0;
	int dir = colecao->tamanho - 1;
	while(esq <= dir){
		meio = (esq + dir) / 2;
		comparacoes++;
		if(strcmp(colecao->restaurantes[meio]->nome, chave) == 0){
			return 1;
		}
		else if(strcmp(colecao->restaurantes[meio]->nome, chave) < 0){
			esq = meio + 1;
		}
		else{
			dir = meio - 1;
...
void generate_log(double tempo){
	FILE *log = fopen("903104_binaria.txt", "w");

	if(log != NULL){
		fprintf(log, "903104\t%ld\t%f",
			comparacoes,
			tempo);
	fclose(log);
	}
...
int main(){
	...
	while(scanf(" %[^\n]", line) == 1){
		...
		if(pesquisa_binaria(filtrados, line)){
			printf("SIM\n");
		}else{
			printf("NAO\n");
		}	
