...
void inserirPilha(Colecao_Restaurantes *c, Restaurante *r){
    if(c->tamanho == c->capacidade){
        c->restaurantes = aumentarCapacidade(c->restaurantes, &c->capacidade);
    }
...
Restaurante * removerPilha(Colecao_Restaurantes *c){
    if(c->tamanho == 0){
        return NULL;
    }
    
    Restaurante *r = c->restaurantes[c->tamanho - 1];
    c->restaurantes[c->tamanho - 1] = NULL;
    c->tamanho--;

    return r;
}

void mostrar(Colecao_Restaurantes *c){
    char buffer[1024];
...
int main(){
    ...
    Colecao_Restaurantes *pilha = malloc(sizeof(Colecao_Restaurantes));
    pilha->tamanho = 0;
    pilha->capacidade = base->tamanho;
    pilha->restaurantes = malloc(pilha->capacidade * sizeof(Restaurante*));
    ...
    while (1) {
        ...
        if (n <= base->tamanho) {
            inserirPilha(pilha, base->restaurantes[n-1]);
        }
    }
    ...
    for(int i = 0; i < m; i++){
        ...
        if(strcmp(comando, "I") == 0){
            ...
            inserirPilha(pilha, base->restaurantes[id - 1]);

        } else if(strcmp(comando, "R") == 0){
            Restaurante *r = removerPilha(pilha);
