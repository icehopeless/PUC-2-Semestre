...
public class filaCircular extends stringUtils {
    private static Restaurante[] fila = new Restaurante[5];
    private static int primeiro = 0;
    private static int ultimo = 0;
    private static int qtde = 0;

    public static void enfileirar(Restaurante r) {
    ...
    public static Restaurante desenfileirar() {
        if (qtde == 0) return null;
        
        Restaurante removido = fila[primeiro];
...
public static int calcularMediaAnos() {
    if (qtde == 0) return 0;
    ...
    for (int c = 0; c < qtde; c++) {
        soma += fila[i].getDataAbertura().getAno();
        i = (i + 1) % 5;
    }
    return (soma + (qtde / 2)) / qtde; 
}

    public static void mostrarFila() {
        int i = primeiro;
        for (int c = 0; c < qtde; c++) {
            System.out.println(fila[i].formatar());
            i = (i + 1) % 5;
        }
    }
    ...
    public static void main(String[] args) {
        ...
        ColecaoRestaurantes colecaoTudo = ColecaoRestaurantes.readCsv();
        Restaurante[] baseDados = colecaoTudo.getRestaurantes();
        filaCircular fila = new filaCircular();

        while (scan.hasNextInt()) {
            int n = scan.nextInt();
            if (n == -1) break;
            fila.enfileirar(baseDados[n - 1]);
        }
        ...
        if (scan.hasNextInt()) {
            ...
            for (int i = 0; i < numOps; i++) {
                ...
                if (comando.compareTo("I") == 0) {
                    int id = scan.nextInt();
                    fila.enfileirar(baseDados[id - 1]);
